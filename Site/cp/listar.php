    ...
    <form id="filter-form" class="small-12 large-6 right" action="<?="?q=".$_GET["q"]."&a=listar"?>" method="post">
                ...
                <div class="small-12 medium-4 left">
                <input type="text" name="d" placeholder="<?=$inline["searchTerm"]?>" />
                </div>
                <div class="small-12 medium-4 left ">
                <input class="small-12 button tiny" type="submit" value="<?=$inline["search"]?>" />
                </div>
    </form>
    <hr />
   	<div class="column small-12">
        <h1><?=$inline["viendo"]?> <?=$secciones[$_GET["q"]]["t"]?></h1>
    </div>
    <hr />
    ...
    <div>
		...
		$txt="<span>$rng</span> ".$inline["results"];
		
		if ($_POST){
			$txt.=", ".$inline["buscando"]." $q: <span>$d</span>";
		}
		...
		if ($_GET["p"]){
			...
			$b=$xpag;
			$limit="LIMIT $a,$b";
			$txt.= " (".$inline["pagina"]." $p ".$inline["of"]." ".ceil($rng/$xpag).")";
		}else{
			$limit="LIMIT 0,$xpag";
			$p=1;
		}
	...
	if ($rn>0){
					...
					if (!isset($val["hide"]) || $val["hide"]<2) {
						...
						if ($val["type"]!="password"){
							if ($val["hide"]==1 ) {
								...
								$popllink='<a href="#" data-reveal-id="i'.$val["db"].$i.'">'.$info["show"].' '.$val["t"].'</a>';
		...
		echo "<p class='alert-box warning '>".str_replace("[:x:]",$secciones[$_GET["q"]]["t"],$inlint["noresults"])."</p>";
	}
    ?>
    </table>

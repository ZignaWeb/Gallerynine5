	...
	<hr />
   	<div class="column small-12">
        <h1><?=$inline["delete"]?>:  <?=$secciones[$_GET["q"]]["t"]?></h1>
    </div>
    <hr />
	...
	$ide=mysql_real_escape_string($_GET["i"]);
		...
		if (borrar($secciones[$que]["db"],$ide)){
			echo '<p>'.$inline["eliminado"].': '.ucfirst($secciones[$que]["t"]).'</p>';
		}else{
			echo '<p>'.$error["tryagain"].'</p>';
		}
	...
	?>
    <a class="button" onclick="javascript:history.go(-1)"><?=$inline["goback"]?></a>
